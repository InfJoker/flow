...
export default function StatePanel({
  ...
  const [pickingAgentIndex, setPickingAgentIndex] = useState<number | null>(null);
  ...
  return (
    <div className="state-panel">
      ...
      {pickingAgentIndex !== null && (
        <AgentPicker
          agents={agents}
          current={(state.actions ?? [])[pickingAgentIndex]?.agent}
          onSelect={(name) => {
            const action = (state.actions ?? [])[pickingAgentIndex];
            if (action) {
              updateAction(pickingAgentIndex, { ...action, agent: name });
            }
            setPickingAgentIndex(null);
          }}
          onClose={() => setPickingAgentIndex(null)}
        />
      )}
    </div>
  );
